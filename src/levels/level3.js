...
// --- Phase 1: Briser la Barrière de Wanda ---
const WORD_TO_FORM = "ESPRIT";
const LETTERS_AVAILABLE = WORD_TO_FORM.split('');
const MAX_ERRORS_PHASE1 = 3;
const ANCHOR_POINT_SIZE = 60;

// --- Phase 2: Extraire la Pierre de l'Esprit ---
...
export function startWakanda() {
    ...
    if (storyButton) {
        ...
        storyButton.onclick = () => {
            ...
            if (isStoryVisible) ui.hideDialogue();
            else ui.showDialogue(narrator, STORY_TEXT_LVL3);
        };
    }
    setupPhase1();
}
...
function setupPhase1() {
    currentPhase = 1;
    ui.showDialogue(wanda, "Tu ne l'auras pas ! Vision est sous ma protection !");
    ...
    if (LETTERS_AVAILABLE.length > anchorPositions.length) {
        console.error("Pas assez de positions définies pour les points d'ancrage ! Mot:", WORD_TO_FORM, "Positions:", anchorPositions.length);
        return;
    }
    ...
    shuffledLetters.forEach((letter, index) => {
        ...
        Object.assign(anchor.style, {
            ...
            top: anchorPositions[index].top, left: anchorPositions[index].left,
...
function handleAnchorClick(clickedLetter, anchorElement) {
    ...
    if (WORD_TO_FORM.startsWith(currentWordFormed)) {
        if (currentWordFormed === WORD_TO_FORM) { victoryPhase1(); }
    } else {
        errorsPhase1++;
        ...
        if (errorsPhase1 >= MAX_ERRORS_PHASE1) { defeatPhase1("Trop d'erreurs ! Sa magie est trop instable !"); return; }
        resetAnchors();
    }
}
...
function resetAnchors() {
    ...
    anchorPoints.forEach(anchor => {
        ...
        anchor.onclick = () => handleAnchorClick(anchor.dataset.letter, anchor);
    });
}
...
function setupPhase2() {
    ...
    Object.assign(piecesContainer.style, {
        position: 'absolute',
        padding: '10px',
        backgroundColor: 'transparent', // MODIFICATION: Fond transparent
        display: 'flex',
        flexWrap: 'wrap', 
        justifyContent: 'flex-start', 
        gap: '8px',
        width: `${PUZZLE_GRID_SIZE * (PUZZLE_PIECE_DISPLAY_SIZE + 8) + 12}px`, 
        zIndex: '50' 
    });
    ...
    for (let i = 0; i < PUZZLE_GRID_SIZE * PUZZLE_GRID_SIZE; i++) {
        ...
        const col = originalIndex % PUZZLE_GRID_SIZE;
        const row = Math.floor(originalIndex / PUZZLE_GRID_SIZE);
...
function handleDropOnSlot(event, slotElement) {
    ...
    if (piecesCont && piecesCont.children.length === 1 && piecesCont.contains(draggedPiece) ) { // S'il ne reste que la pièce en cours de déplacement (qui va être ajoutée au slot)
         // Note: cette logique est imparfaite si on prend une pièce déjà dans un slot pour la remettre dans le container.
         // Mieux vaut vérifier le nombre d'enfants après l'appendChild potentiel ci-dessus.
    } else if (piecesCont && (piecesCont.children.length - (piecesCont.contains(draggedPiece) ? 1:0) === 0)) {
         // Masquer si le conteneur est vide après le déplacement
...
function checkPuzzleComplete() {
    // Après chaque drop, vérifier si le conteneur de pièces est vide.
    const piecesCont = document.getElementById('puzzle-pieces-container');
    let allPiecesPlaced = true;
    if (piecesCont) {
        for(let i=0; i < piecesCont.children.length; i++){
            // Vérifier si les enfants restants sont bien des pièces non placées (au cas où d'autres éléments y seraient)
    ...
    if (allPiecesPlaced && piecesCont) { // Si toutes les pièces ont été déplacées du conteneur
        piecesCont.style.display = 'none'; // Cacher le conteneur
    }


    for (let slot of puzzleSlots) {
        if (!slot.firstChild || slot.firstChild.dataset.pieceId !== slot.dataset.expectedPiece) { return false; }
    }
    victoryPhase2();
    return true;
}
...
function victoryPhase2() {
    ...
    const piecesCont = document.getElementById('puzzle-pieces-container');
    if (piecesCont && piecesCont.parentNode) piecesCont.remove(); // On le supprime complètement ici
    ...
    Object.assign(revealedPuzzleImage.style, {
        position: 'absolute', top: '50%', left: '50%',
        transform: `translate(calc(-50% - ${puzzleTotalSize / 3}px), -50%)`, // Doit correspondre au transform de puzzleArea
        width: `${puzzleTotalSize}px`, height: `${puzzleTotalSize}px`,
